                        ...
                        <form method="post" action="{{ route('users.setAccess',$user->id) }}">
                            ...
                            <div class="card-body">
                                <ul class="nav nav-tabs " role="tablist">
                                    ...
                                    <li class="nav-item">
                                        <a class="nav-link" data-toggle="tab" href="#tabs-{{$branch->id}}" role="tab">
                                ...
                                <div class="tab-content">
                                    @foreach ($branches as $key => $branch)
                                    <div class="tab-pane" id="tabs-{{$branch->id}}" role="tabpanel">
                                        @foreach($sections as $section)
                                        {{-- Admin Filter --}}
                                        @if (
                                        in_array($user->role,['sales','noc','finance']) &&
                                        in_array($section->section,['users','reseller','field-officers','province','branch','category'])
                                        )
                                        @php continue @endphp
                                        @endif
                                        ...
                                        {{-- NOC Filter --}}
                                        @if (
                                        in_array($user->role,['sales','finance']) &&
                                        in_array($section->section,['installation','provincial-installation'])
                                        )
                                        @php continue @endphp
                                        @endif
                                        ...
                                        {{-- NOC Filter --}}
                                        @if (
                                        in_array($user->role,['sales','noc']) &&
                                        in_array($section->section,['payment','provincial-payment'])
                                        )
                                        @php continue @endphp
                                        @endif
                                        ...
                                        {{-- NOC & FINANCE Filter --}}
                                        @if (
                                        in_array($user->role,['finance','noc']) &&
                                        in_array($section->section,['package'])
                                        )
                                        @php continue @endphp
                                        @endif
                                            ...
                                            <div class="col-sm-12 px-16">
                                                <div class="parent">
                                                    <label for="{{ $section->section }}">
                                                        ...
                                                        -
                                                        <span>{{ headline($section->section) }}</span>
                                                    </label>
                                                </div> <!-- /parent -->
                                                ...
                                                <div>
                                                    <ul class="child">
                                                        ...
                                                        <li>
                                                            <label for="{{$branch->id.'-'.$permission->id }}">
                                                                <input type="checkbox"
                                                                    name="permission[{{$branch->id}}][]"
                                                                    value="{{ $permission->id }}"
                                                                    id="{{ $branch->id.'-'.$permission->id }}"
                                                                ...
                                                                -
                                                                <span>
                                                                    {{ headline($permission->permission) }}
                                                                </span>
